...
**Step 1 — Only published profiles compete.** Draft profiles are excluded from winner selection entirely. They still appear in the waterfall audit trail (marked `rejected`) so the supplier can see what *would* apply if published, but they never affect the price.
...
This gives the supplier full transparency into *why* each product costs what it does.
