      ...
      {price.waterfall.length > 0 && (
        <div>
            ...
            <table className="w-full text-[13px]">
              <thead>
                <tr className="border-b border-surface-border bg-gray-100/60">
                  ...
                  <th className="px-3 py-2 text-right font-medium text-ink-500 whitespace-nowrap">Floor Price</th>
                ...
                {price.waterfall.map((w: WaterfallEntry) => (
                  <tr key={w.profileId} className="border-b border-surface-border-soft last:border-b-0">
                    ...
                    <td className="px-3 py-2 text-center">
                      {w.tier != null ? (
                        <span className="rounded-pill bg-indigo-50 px-1.5 py-0.5 text-[11px] font-medium text-indigo-700">T{w.tier}</span>
                      ) : <span className="text-ink-400">&mdash;</span>}
                    </td>
                    <td className="px-3 py-2 text-right font-semibold text-ink-900 whitespace-nowrap">${w.computedPrice.toFixed(2)}</td>
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      {w.priceAfterFloor != null
                        ? <span className="font-medium text-amber-700">${w.priceAfterFloor.toFixed(2)}</span>
                        : <span className="text-ink-400">&mdash;</span>}
                    </td>
                    <td className="px-3 py-2 text-center"><span className={verdictClass(w.verdict)}>{w.verdict}</span></td>
          ...
          <div className="md:hidden space-y-2">
            {price.waterfall.map((w: WaterfallEntry) => (
              <div key={w.profileId} className="rounded-lg border border-surface-border-soft bg-white p-3 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium text-ink-900">#{w.position} {w.profileName}</span>
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-ink-600">
                  ...
                  <span className="font-semibold text-ink-900">${w.computedPrice.toFixed(2)}</span>
                  {w.priceAfterFloor != null && (
                    <span className="font-medium text-amber-700">Floor: ${w.priceAfterFloor.toFixed(2)}</span>
                  )}
                </div>
                  ...
                  <table className="w-full min-w-[800px]">
                    <thead>
                      <tr className="border-b border-surface-border">
                        ...
                        <th className="px-4 py-3 text-left text-[13px] font-medium text-ink-500">Product</th>
                        <th className="px-4 py-3 text-right text-[13px] font-medium text-ink-500 whitespace-nowrap">Base Price</th>
                        <th className="px-4 py-3 text-right text-[13px] font-medium text-ink-500 whitespace-nowrap">Final Price</th>
                        <th className="px-4 py-3 text-right text-[13px] font-medium text-ink-500">Difference</th>
                        <th className="px-4 py-3 text-right text-[13px] font-medium text-ink-500">Floor</th>
                        <th className="px-4 py-3 text-left text-[13px] font-medium text-ink-500 whitespace-nowrap">Applied Profile</th>
                        <th className="px-4 py-3 text-center text-[13px] font-medium text-ink-500">Tier</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <Fragment key={price.productId}>
                            <tr
                              ...
                              onClick={() => hasProfile && toggleExpand(price.productId)}
                            >
                              ...
                              <td className="px-4 py-4 text-sm font-medium text-ink-900">{price.productTitle}</td>
                              <td className="px-4 py-4 text-right text-sm text-ink-700 whitespace-nowrap">${price.basePrice.toFixed(2)}</td>
                              <td className="px-4 py-4 text-right text-sm font-semibold text-ink-900 whitespace-nowrap">${price.finalPrice.toFixed(2)}</td>
                              ...
                              <td className="px-4 py-4 text-right text-sm whitespace-nowrap">
                                {price.floorPrice != null ? (
                                  <span className={price.floorApplied
                                    ...
                                    ${price.floorPrice.toFixed(2)}{price.floorApplied && ' (applied)'}
                                  </span>
                                ) : <span className="text-ink-400">&mdash;</span>}
                              </td>
                              ...
                              <td className="px-4 py-4 text-center whitespace-nowrap">
                                {price.tier ? (
                                  <span className="rounded-pill bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700">Tier {price.tier}</span>
                                ) : <span className="text-ink-400">&mdash;</span>}
                              </td>
                            </tr>
                            {/* Expanded detail row */}
                      ...
                      <div key={price.productId}>
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <h3 className="text-sm font-medium text-ink-900 leading-snug">{price.productTitle}</h3>
                            {hasProfile && (
                              isExpanded
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
                            <span className="text-ink-500">Base: <span className="font-medium text-ink-700">${price.basePrice.toFixed(2)}</span></span>
                            <span className="text-ink-500">Final: <span className="font-semibold text-ink-900">${price.finalPrice.toFixed(2)}</span></span>
                            {hasProfile && (
                              <span className={`font-medium ${diff >= 0 ? 'text-accent-green' : 'text-red-500'}`}>
                                {diff >= 0 ? '+' : ''}{diff.toFixed(2)}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="mt-2 flex flex-wrap items-center gap-2">
                            ...
                            {price.floorPrice != null && (
                              <span className={price.floorApplied
                                ...
                                Floor: ${price.floorPrice.toFixed(2)}{price.floorApplied && ' (applied)'}
                              </span>
                            )}
                            {!hasProfile && <span className="text-xs text-ink-400">No profile applied</span>}
                          </div>
                        </div>
