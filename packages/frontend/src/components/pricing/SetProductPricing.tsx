...
export function SetProductPricing({
  ...
  const { categories, segments, brands } = useMemo(() => {
    return {
      categories: [
        ...new Set(products.map((p: Product) => p.subCategory)),
      ].sort(),
  ...
  const selectedCount =
    scope === "all" ? allProducts.length : selectedProductIds.size;
  ...
  const { data: previewData, isFetching: isPreviewLoading } =
    usePreviewPrices(previewPayload);
  ...
  const customPreviewRows: PreviewPrice[] = useMemo(() => {
    ...
    return selectedProducts.map((product: Product) => ({
      ...
      adjustment:
        (customPrices[product.id] ?? product.basePrice) - product.basePrice,
  ...
  const isComplete = isCustom
    ? selectedCount > 0 &&
      Array.from(selectedProductIds).every((id) => id in customPrices)
                ...
                {scope === "all" ? (
                  ...
                  <>
                    ...
                    {selectedProductNames.length > 3 &&
                      ` & ${selectedProductNames.length - 3} more`}
                  </>
                )}
              </p>
              <p className="text-[13px] text-ink-500">
      ...
      <div>
        ...
        <RadioGroup
          options={[
            ...
            ...(isCustom ? [] : [{ label: "All Products", value: "all" }]),
          ]}
        ...
        {isCustom && (
          <p className="mt-2 text-xs text-ink-400">
            "All Products" is not available with custom pricing -prices must be
            set per product.
          </p>
        )}
      </div>
      ...
      {scope === "all" && !isCustom && (
        <div className="mt-6 rounded-lg border border-teal-200 bg-teal-50 px-4 py-3">
          ...
          <p className="mt-1 text-[13px] text-teal-600">
            This profile will apply to all current and future products. No
        ...
        <div className="mt-4 flex items-start gap-2 rounded-lg border border-blue-200 bg-blue-50 px-4 py-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[13px] text-blue-600">
              Enter an exact price for each product below. No formula or
              calculation is applied -the price you type is the final price.
            </p>
          </div>
        </div>
      )}
      ...
      {!isCustom && (
        <div className="mt-6">
          ...
          <RadioGroup
            ...
            onChange={(val) =>
              onAdjustmentDirectionChange(val as "increase" | "decrease")
            }
          />
        </div>
      )}
        ...
        <div className="mt-6">
          ...
          <div className="relative w-[200px]">
            <input
              ...
              placeholder={
                adjustmentType === "dynamic" ? "e.g. 10" : "e.g. 5.00"
              }
